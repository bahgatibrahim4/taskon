# add-bahgat-subdomain.ps1
# يجب تشغيله كمسؤول (Run as Administrator)

Write-Host "`n🔧 إضافة النطاق الفرعي bahgat.taskon.local..." -ForegroundColor Cyan
...
if ($hostsContent -match "bahgat.taskon.local") {
    Write-Host "⚠️  النطاق موجود بالفعل في ملف hosts" -ForegroundColor Yellow
} else {
    # إضافة النطاق
    Add-Content -Path $hostsPath -Value "127.0.0.1 bahgat.taskon.local"
    Write-Host "✅ تمت إضافة: bahgat.taskon.local" -ForegroundColor Green
}
...
Write-Host "🌐 يمكنك الآن فتح: http://bahgat.taskon.local:4000" -ForegroundColor Cyan
